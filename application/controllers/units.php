...
class Units extends MY_Controller {
    ...
    public function extract_id() {
        ...
        if (strlen($_POST['materials'][1]['content']) > 5) {   // this ensures something is in the field so it only checks if something is there
    ...
    public function chk_form() {      // returns true/false
        ...
        if (strlen($_POST['materials'][1]['content']) > 5) {   // validation rule for 1st supporting material
            $this->form_validation->set_rules('materials[1][content]', 'Supporting Material 1', 'xss_clean|is_valid_url|real_url');
        }
        if (strlen($_POST['materials'][2]['content']) > 5) {   // validation rule for 2nd supporting material
            $this->form_validation->set_rules('materials[2][content]', 'Supporting Material 2', 'xss_clean|is_valid_url|real_url');
        }
        ...
        $this->form_validation->set_message('valid_content', 'Invalid Content. Must be video link or URL to PDF');
